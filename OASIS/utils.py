...
class OlddetJac:
    ...
    def loss(self, pos_flow):
        ...
        if dims == 3:
            ...
            determ = torch.det(stacked) - 0
        return determ  # torch.mean(abs(torch.log(torch.clip(determ, min=0.00001))))
...
class NSETransformer(nn.Module):
    ...
    def __init__(self, size, ndims=3, mode='bilinear', device='cuda'):
        super().__init__()
        self.mode = mode
    ...
    def forward(self, src, flow, device='cuda', padding='border'):
        # new locations
        ...
        return F.grid_sample(src.reshape(1, shape[0], shape[1], shape[2],-1).permute(0,4,1,2,3), new_locs, align_corners=True,
                                   mode=self.mode, padding_mode=padding), new_loc.permute(0,4,1,2,3), self.grid
...
class NSimTransformer(nn.Module):
    ...
    def forward(self, src, flow, device='cuda', padding='border'):
        ...
        if len(shape) == 2:
            ...
            return F.grid_sample(src.reshape(1, -1, shape[0], shape[1]), new_locs, align_corners=True, mode=mode,
...
class register_model(nn.Module):
    def __init__(self, img_size=(64, 256, 256), mode='bilinear'):
        ...
        self.spatial_trans = SpatialTransformer(img_size, mode=mode)
...
class register_modelSE3(nn.Module):
    def __init__(self, img_size=(64, 256, 256), mode='nearest'):
        super(register_modelSE3, self).__init__()
        self.spatial_trans = NSETransformer(img_size, mode=mode)

    def forward(self, x):
        img = x[0].cuda()
        flow = x[1].cuda()
        out,_,_ = self.spatial_trans(img, SE3.exp(flow.reshape(-1,6)))
        return out
    
class register_modelSim3(nn.Module):
    def __init__(self, img_size=(64, 256, 256), mode='nearest'):
        super(register_modelSim3, self).__init__()
        self.spatial_trans = NSimTransformer(img_size, mode=mode)

    def forward(self, x):
        img = x[0].cuda()
        flow = x[1].cuda()
        out,_,_ = self.spatial_trans(img, Sim3.exp(flow.reshape(-1,7)))
        return out
