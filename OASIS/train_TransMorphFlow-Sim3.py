...
def main():
    ...
    best_dsc = 0
    writer = SummaryWriter(log_dir='/results/tensorboard/NCC_SIM3_flowreg5.4')
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            def_segs = []
            for i in range(36):
                def_seg,_,_ = model.spatial_transformer(x_seg_oh[:, i:i + 1, ...].float(), Sim3.exp(output[2].reshape(-1,7)),device='cuda')
            ...
            def_segs = []
            for i in range(36):
                def_seg,_,_ = model.spatial_transformer(y_seg_oh[:, i:i + 1, ...].float(), Sim3.exp(output[2].reshape(-1,7)),device='cuda')
                def_segs.append(def_seg)
            def_seg = torch.cat(def_segs, dim=1)
    ...
    '''
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                for i in range(36):
                    ...
                    def_segs.append(def_seg)
                def_seg = torch.cat(def_segs, dim=1)
                def_grid = reg_model_bilin([grid_img.float(), output[3].cuda()])
                def_out = reg_model([x_seg.cuda().float(), output[3].cuda()])
                dsc = 1-criterion_dsc(def_seg, y_seg.long()) #utils.dice_val_VOI(def_seg, x_seg.long())
                eval_dsc.update(dsc.item(), x.size(0))
                def_out = reg_model([x_seg.cuda().float(), output[3].cuda()])
                def_grid = reg_model_bilin([grid_img.float(), output[3].cuda()])
                print(eval_dsc.avg)
        ...
        save_checkpoint({
            ...
            'best_dsc': best_dsc,
            'optimizer': optimizer.state_dict(),
        }, save_dir='/results/experiments/'+save_dir, filename='dsc{:.3f}.pth.tar'.format(eval_dsc.avg))
        writer.add_scalar('DSC/validate', eval_dsc.avg, epoch)
        plt.switch_backend('agg')
        pred_fig = comput_fig(def_out)
        grid_fig = comput_fig(def_grid)
