...
def main():
    ...
    train_dir = '/data/IXI_data/Train/'
    val_dir = '/data/IXI_data/Val/'
    save_dir = '/TransMorph_original/'
    ...
    Initialize model
    '''
    config =  get_3DTransMorphLarge_config()
    ...
    best_dsc = 0
    writer = SummaryWriter(log_dir='/results/tensorboard/run6')
    for epoch in range(epoch_start, max_epoch):
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                output = model(x_in)
                def_out = reg_model_bilin([x_seg.cuda().float(), output[1].cuda()])
