...
class TransMorphDiff(nn.Module):
    ...
    def __init__(self, config):
        ...
        if not config.downsample:
            self.flow_vol_shape = self.img_sz
        ...
        self.downsample = False #config.downsample
        if not config.downsample:
            self.reg_res_id_transform = gen_identity_map(self.img_sz, 1.0, normalized=False).cuda()
        ...
        if not self.downsample:
            self.up3 = TM.DecoderBlock(embed_dim, embed_dim // 2, skip_channels=embed_dim // 2 if if_convskip else 0,
                                use_batchnorm=False)
            self.up4 = TM.DecoderBlock(embed_dim // 2, config.reg_head_chan, skip_channels=config.reg_head_chan if if_convskip else 0,
                                use_batchnorm=False)
            self.c2 = TM.Conv3dReLU(2, config.reg_head_chan, 3, 1, use_batchnorm=False)
    ...
    def forward(self, inputs):
        source, target = inputs[:,0:1],inputs[:,1:]
        ...
        if False:
            ...
            flow = 2**(-self.int_steps)*flow_mean
...
CONFIGS = {
    ...
    'TransMorphDiff-Org-Res': configs.get_TransMorphDiffOrgRes_config(),
}